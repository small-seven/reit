...
def load_pretrained(model, default_cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True, progress=False):
    ...
    # try:
    path = f'{args_vit.proj_dir}/checkpoints/pretrained/{args_vit.model}.pth'
    state_dict = torch.load(path, map_location='cpu')
    _logger.warning('Use local pretrained checkpoint!')
    # except:
    #     _logger.warning('go into except!')
...
def swin_checkpoint_filter_fn(state_dict, model, args):
    ...
    if args.crop == 32 and 'CIFAR' in args.dataset:
        down_indicator = True
    else:
        down_indicator = False
    _logger.warning(f"down_indicator={down_indicator} dataset={args.dataset}")
    for k, v in state_dict.items():
        ...
        if down_indicator and 'attn.relative_position_bias_table' in k:
            # names = k.split('.')
            # layer = int(names[1])
            # block = int(names[3])
            # v = model.layers[layer].blocks[block].attn.relative_position_bias_table
            if 'layers.3.blocks.' in k:
                dim1 = (2 * args.patch - 1) * (2 * args.patch - 1)
                v = v[:dim1, :]
            else:
                dim1 = (2 * args.window_size - 1) * (2 * args.window_size - 1)
                v = v[:dim1, :]
        if down_indicator and 'attn.relative_position_index' in k:
            names = k.split('.')
            layer = int(names[1])
            block = int(names[3])
            v = model.layers[layer].blocks[block].attn.relative_position_index
            # if 'layers.3.blocks.' in k:
            #     dim = args.patch ** 2
            #     v = v[:dim, :dim]
            # else:
            #     dim = args.window_size ** 2
            #     v = v[:dim, :dim]
        if down_indicator and 'blocks.1.attn_mask' in k:
            names = k.split('.')
            layer = int(names[1])
            block = int(names[3])
            v = model.layers[layer].blocks[block].attn_mask
            # if 'layers.0.' in k:
            #     dim = args.window_size ** 2
            #     v = v[:16, :dim, :dim]
            # elif 'layers.1.' in k:
    ...
    if pretrained:
        if pretrained_custom_load:
            ...
            if pretrained_filter_fn is not None:
                checkpoint_filter_fn = pretrained_filter_fn
            elif 'swin' in args_vit.model:
                checkpoint_filter_fn = partial(swin_checkpoint_filter_fn, args=kwargs.pop('args'), model=model)
            else:
                raise ValueError(args_vit.model)
            load_pretrained(
                model,
