...
`How to train_vit your ViT? Data, Augmentation, and Regularization in Vision Transformers`
...
from timm_vit.helpers import build_model_with_cfg, named_apply, adapt_input_conv
...
args_deit = Train_Config(get_args())
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # our method
        if args_deit.r_std != 0.0:
            r = args_deit.r_std * torch.randn(B, 1, C).to(x.device)
            if args_deit.eval and not self.training:
    ...
    def get_R_testing(self, x, r1):
        ...
        r2 = args_deit.r_std * torch.randn(B, 1, C).to(x.device)

        # r2_data = args_deit.r_std * torch.randn(B, 1, C).to(x.device)
        sim_min, r2_best = 1., r2.data
        # r2 = torch.zeros_like(r2_data).to(x.device)
        for i in range(args_deit.r_steps):
            ...
            with torch.no_grad():
                r2.data -= args_deit.r_alpha * torch.sign(r2.grad.detach())
                r2_std = torch.std(r2.data, dim=-1, keepdim=True)
                r2_mean = torch.mean(r2.data, dim=-1, keepdim=True)
                r2.data = args_deit.r_std * (r2.data - r2_mean) / r2_std
                # r2_data = r2.data
                r2.grad.zero_()
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.data_norm = DataNorm(mean=args_deit.data_mean, std=args_deit.data_std)
