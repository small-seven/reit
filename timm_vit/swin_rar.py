...
sys.path.append("..")
from timm_vit.helpers import overlay_external_default_cfg, build_model_with_cfg
...
args_swin = Train_Config(get_args())
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        # print(self.relative_position_bias_table.size())
        # print(self.relative_position_index.size())
        # print(self.relative_position_index.view(-1).size())
        # print(self.window_size[0])
        # print(self.window_size[1])
        ...
        # our method
        if args_swin.r_std != 0.0:
            r = args_swin.r_std * torch.randn(B_, 1, C).to(x.device)
            if args_swin.eval and not self.training:
    ...
    def get_R_testing(self, x, r1):
        B, N, C = x.shape
        r2_data = args_swin.r_std * torch.randn(B, 1, C).to(x.device)

        sim_min, r2_best = 1., r2_data
        for i in range(args_swin.r_steps):
            ...
            # X_grad_R2 = torch.autograd.grad(R2.mean(), x_new, retain_graph=True)
            ...
            # r2_grad = torch.autograd.grad(loss_sim_XgradR, r2, retain_graph=False, create_graph=False)
            with torch.no_grad():
                # loss_sim_XgradR_data = loss_sim_XgradR.item()
                # del R1, R2, X_grad_R1, X_grad_R2, loss_sim_XgradR, x_new
                r2.data -= args_swin.r_alpha * torch.sign(r2.grad.detach())
                # r2.data -= args_vit.r_alpha * torch.sign(torch.tensor(r2_grad[0]))
                r2_std = torch.std(r2.data, dim=-1, keepdim=True)
                r2_mean = torch.mean(r2.data, dim=-1, keepdim=True)
                r2.data = args_swin.r_std * (r2.data - r2_mean) / r2_std
                r2_data = r2.data
                r2.grad.zero_()
                # del r2
                # r2.grad.zero_()
                ...
                # if sim_min > loss_sim_XgradR_data:
                #     sim_min = loss_sim_XgradR_data
                #     r2_best = r2_data
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.data_norm = DataNorm(mean=args_swin.data_mean, std=args_swin.data_std)
...
def swin_base_patch4_window7_224(pretrained=False, patch_size=4, window_size=7, **kwargs):
    """ Swin-B @ 224x224, pretrained ImageNet-22k, fine tune 1k
    """
    model_kwargs = dict(
        patch_size=patch_size, window_size=window_size, embed_dim=128, depths=(2, 2, 18, 2), num_heads=(4, 8, 16, 32),
        **kwargs)
...
def swin_small_patch4_window7_224(pretrained=False, patch_size=4, window_size=7, **kwargs):
    """ Swin-S @ 224x224, trained ImageNet-1k
    """
    model_kwargs = dict(patch_size=patch_size, window_size=window_size, embed_dim=96,
                        depths=(2, 2, 18, 2), num_heads=(3, 6, 12, 24), **kwargs)
    return _create_swin_transformer('swin_small_patch4_window7_224', pretrained=pretrained, **model_kwargs)


def swin_tiny_patch4_window7_224(pretrained=False, patch_size=4, window_size=7, **kwargs):
    """ Swin-T @ 224x224, trained ImageNet-1k
    """
    model_kwargs = dict(patch_size=patch_size, window_size=window_size, embed_dim=96,
                        depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), **kwargs)
