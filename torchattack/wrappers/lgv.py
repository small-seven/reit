...
class LGV(Attack):
    ...
    Arguments:
        ...
        trainloader (torch.utils.data.DataLoader): data loader of the unnormalized train_vit set. Must load data in [0, 1].
    ...
    Examples::
        >>> attack = torchattack.LGV(model, trainloader, lr=0.05, epochs=10, nb_models_epoch=4, wd=1e-4, n_grad=1, attack_class=BIM, eps=4/255, alpha=4/255/10, steps=50, verbose=True)
    ...
    def __init__(self, model, trainloader, lr=0.05, epochs=10, nb_models_epoch=4, wd=1e-4, n_grad=1, verbose=True,
        ...
        model = copy.deepcopy(model)  # deep copy the model to train_vit it
