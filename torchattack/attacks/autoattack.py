...
class AutoAttack(Attack):
    ...
    Examples::
        >>> attack = torchattack.AutoAttack(model, norm='Linf', eps=8/255, version='standard', n_classes=10, seed=None, verbose=False)
    ...
    def __init__(self, model, norm='Linf', eps=8 / 255, version='standard', n_classes=10, seed=None, verbose=False,
        ...
        if version == 'standard':  # ['apgd-ce', 'apgd-t', 'fab-t', 'square']
            self._autoattack = MultiAttack([
                ...
                APGDT(model, eps=eps, norm=norm, seed=self.get_seed(), verbose=verbose, n_classes=n_classes,
                      n_restarts=1),
                FAB(model, eps=eps, norm=norm, seed=self.get_seed(), verbose=verbose, multi_targeted=True,
                    n_classes=n_classes, n_restarts=1),
                Square(model, eps=eps, norm=norm, seed=self.get_seed(), verbose=verbose, n_queries=n_queries, n_restarts=1),
            ])
            ...
            self._autoattack = MultiAttack([
                ...
                FAB(model, eps=eps, norm=norm, seed=self.get_seed(), verbose=verbose, n_classes=n_classes,
                    n_restarts=5),
            ...
            self._autoattack = MultiAttack([
                APGD(model, eps=eps, norm=norm, seed=self.get_seed(), verbose=verbose, loss='ce', eot_iter=20,
                     n_restarts=1),
                APGD(model, eps=eps, norm=norm, seed=self.get_seed(), verbose=verbose, loss='dlr', eot_iter=20,
                     n_restarts=1),
            ])
